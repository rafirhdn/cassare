<template>
   <div class="font-mono">
      <!-- Wrapper -->
      <div class="w-full border-b border-dark-theme-800 px-4 py-4 justify-between flex flex-row">
         <!-- Title -->
         <div>
         ...
         <!-- Button -->
         <button @click="exportExcel" class="bg-dark-theme-50 rounded-sm flex flex-row gap-2 py-1 px-4 hover:bg-dark-theme-400 text-dark-theme-950 hover:cursor-pointer text-sm font-normal tracking-tight items-center">
            <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
               <rect width="24" height="24" fill="none" />
...
const exportExcel = () => {
   ...
   const namaAkun = carts.value[0]?.admin_name ?? 'Kasir'
   const namaFile = namaAkun.toLowerCase().replace(/\s+/g, '-')
   ...
   XLSX.utils.sheet_add_aoa(worksheet, [[`Laporan Transaksi - ${namaAkun}`]], { origin: 'A1' })
   ...
   XLSX.writeFile(workbook, `Laporan_Transaksi_${namaFile}.xlsx`, { cellStyles: true })
}
...
// Computed: group transaksi per hari
